...
> **⚠️ AI生成シナリオはアルファ版です**
> LLMによるシナリオ自動生成機能は現在開発中です。構造的な不整合（詰み・ヒント不足など）が発生する場合があります。
> 安定して動作するシナリオを使いたい場合は `--scenario scenarios/cursed_library.json` を指定してください。
...
- **WARNING文の修正**: 死亡描写をそのまま warning に転用しないよう、`memory_hint` を優先して使用
- **シナリオ整合性チェック強化**: 以下のケースを自動修復またはエラー検出してリトライ
  - `lock_id` が存在しない錠前を参照しているアイテム → `lock_id` を除去
  - `code_limits.exhaust_trap` が存在しないトラップを参照 → `exhaust_trap` を除去
  - 最終鍵が `contains` 経由（examine で取り出せる）で入手可能 → エラー
  - 錠前を `lock_id` で参照しているアイテムが存在しない → エラー
- **BFS修正**: `reward_only` アイテムを初期 visible から除外し、実際のゲーム開始状態と一致させた
- **examine テキストのルール追加**: 「埃で読めない」など別アイテムの使用を示唆して情報を出し渋ることを禁止

**AIプレイヤー改善**
- **錠前種別の明示**: AIプロンプトに錠前の種別（`N桁の数字錠→enter_code` / `物理鍵錠→use_item`）を表示
- **行動ルールの追加**: `look_around` の無駄な繰り返し・`use_item` の無意味な組み合わせ試行を抑制するルールをプロンプトに追加
- **やったことメモ**: マップビューアの下部に部屋ごとの行動ログを日本語で表示（重複行動の回数も表示）。AIプロンプトにも「すでに試したこと」として反映

**その他**
- **マルチルームキャンペーン**: `--rooms N` で複数部屋を連続プレイ。`--hardcore` で記憶を部屋またぎで引き継ぐ
- **JSON修復**: LLMがトレーリングカンマを含む不正JSONを返した場合に自動修復
- **WARNING罠の改善**: 警告系の罠が `examine` 結果を隠さないよう修正
